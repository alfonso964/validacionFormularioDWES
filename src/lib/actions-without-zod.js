...
const schema = z.object({
    ...
    edad: z.coerce.number().min(16, "La edad mínima debe ser 16 años").max(70, "La edad máxima debe ser 70 años"),
    telefono: z.string().trim().regex(/^[679][0-9]{8}$/, "Escribe 9 dígitos, siendo el primero 6,7 o 9"),
    email: z.string().email({ message: "Email no válido" }),
    fecha: z.coerce.date()
        .min(new Date("2024-01-01"), "La fecha debe ser posterior al 1 de enero 2024")
        .max(new Date(), "La fecha no puede ser futura"),
    comentario: z.string().min(10, "El comentario debe tener al menos 10 caracteres").max(500, "Máximo 500 caracteres permitidos").optional()
});
